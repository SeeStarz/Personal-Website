...
└── src
    ...
    └── ts/
```
Before the project is built, `dist/` and `env/` are not present.
- LICENSE.txt contains MIT license text.
- `env/` is optional if python virtual environment is used instead of global install. \[not tracked by git\]
- `shell.nix` describes required dependencies and environment setup.
- `requirements.lock` contains exact versions of python packages.
...
#### Environment Setup
There's nothing really. Optionally you can use the `requirements.lock` to install `jinja2` that way, could be with a virtual environment could be not, up to you.
